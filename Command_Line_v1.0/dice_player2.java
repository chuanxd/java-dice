...
public static void main(String[] args)
{
  String userName;String betStr;String result = new String("");
	...
	//write player 2 name. Filename is player2.txt 
	try {
	FileOutputStream fos = new FileOutputStream(".//player2.txt");
	...
	try {
	FileOutputStream fos = new FileOutputStream(".//player2dicesum.txt");
	BufferedOutputStream bos = new BufferedOutputStream(fos);
	PrintStream ps = new PrintStream(bos);
	ps.println(diceSum);
	ps.close();
	...
	File sumPosition = new File("player1dicesum.txt");
	int player1dicesum = 0;
	try{
	BufferedReader input = new BufferedReader(new FileReader(sumPosition) );
	player1dicesum = Integer.parseInt(input.readLine());
	System.out.println("Player 1 total-point is "+player1dicesum+"."+"\n==========================================");
	}
	catch(Exception e)
	{System.out.print("Something wrong!(Read File)");}
	//Open Answer
	result = "You Loss!!!";
	if(player1dicesum <10 && betStr.equals("Small")){
		result = "You Win!!!";
	}
	if(player1dicesum >10 && betStr.equals("Large")){
		result = "You Win!!!";
	}
